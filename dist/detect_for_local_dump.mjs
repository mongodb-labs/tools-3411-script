...
var METADATA_FILE_SUFFIX = ".metadata.json";
var METADATA_FILE_SUFFIX_GZ = ".metadata.json.gz";
function parseDirectory(directory) {
  const metadataFiles = getMetadataFiles(directory);
  console.log(`Found ${metadataFiles.length} metadata files.`);
  return metadataFiles.map((file) => {
    let filePath = `${directory}/${file}`;
    let buffer2 = readFileSync(filePath);
    if (filePath.endsWith(METADATA_FILE_SUFFIX_GZ)) {
      buffer2 = gunzipSync(buffer2);
    }
...
function getMetadataFiles(directory) {
  try {
...
function isMetadataFile(file) {
  return file.endsWith(METADATA_FILE_SUFFIX) || file.endsWith(METADATA_FILE_SUFFIX_GZ);
}
...
var ARCHIVE_SUFFIX_GZ = ".gz";
var BYTES_IN_INT32 = 4;
var TERMINATOR = 4294967295;
var MAGIC_NUMBER = 1843566977;
var pipelineAsync = promisify(pipeline);
var ParseArchiveStream = class extends Transform {
  source;
...
async function parseArchive(archiveFilepath) {
...
function createArchiveStreams(archiveFilepath) {
  const readStream = createReadStream(archiveFilepath);
  const gunzipStream = createGunzip();
  return archiveFilepath.endsWith(ARCHIVE_SUFFIX_GZ) ? [readStream, gunzipStream] : [readStream];
}
async function collectStreamObjects(objects, stream) {
  for await (const obj of stream) {
    objects.push(obj);
  }
}
function readDocumentMetadata(buffer2, documentLength) {
  const documentBytes = buffer2.subarray(0, documentLength);
  const document = bson.deserialize(documentBytes);
  const metadata = document.metadata == null ? {} : EJSON.parse(document.metadata);
  return {
...
function int32ToBytes(n) {
  const buffer2 = Buffer.alloc(BYTES_IN_INT32);
  buffer2.writeUInt32BE(n, 0);
  return buffer2;
}
...
async function getMetadata({ mode, path: path2 }) {
  switch (mode) {
    case "directory":
      return parseDirectory(path2);
    case "archive":
      return parseArchive(path2);
    default:
